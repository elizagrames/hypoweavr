...
input.data <- read.csv("./data/screened_pathdata.csv")

# Subset to only the first 30% random sample
input.data <- input.data[1:296,]
...
sp::plot(usa, add=F, border="grey85")

points(studysites, col="white", pch=20, lwd=0, cex=1.75)
points(studysites, col=paste(palette[1], "bb", sep=""), pch=20, cex=1.75, lwd=0)
sb <- sp::layout.scale.bar(height = 100)
# shift scale bar
sb_slide <- maptools::elide(sb, shift = c(raster::extent(forested)[1],raster::extent(forested)[3]-0.1))
plot(sb_slide, add = TRUE, col = c("white","black"))
plot(sb_slide)
library(raster)
points(0,0)
sp::plot(forested, col=paste(palette[4], "22", sep=""), axes=F, box=F, legend=F)
sb <- raster::scalebar(d = 2500000, # distance in km
                 xy = c(-300000,0),
                 type = "bar", 
                 divs = 4, 
                 below = "Mm", 
...
sort(igraph::strength(metaDAG))

# Plot graph and rearrange layout
tkplot(metaDAG, 
            edge.arrow.size=.7, 
            edge.color=paste(palette[4], "88", sep=""),
            vertex.color="#ffffff00", 
            vertex.frame.color="#ffffff00", 
            vertex.label.family="Arial",
            vertex.label.dist=0, 
            vertex.label.color="black", 
            vertex.label.cex=1, layout=layout_with_dh(metaDAG),
       vertex.size=sqrt(strength(metaDAG))*3,
            edge.width=as.numeric(lookup_widths(study_DAGS, metaDAG))/2,
            )
...
edgedf[,1] <- factor(append(unlist(lapply(strsplit(names(edgedat), "\\|"), function(x){x[1]})), unlist(names(igraph::V(metaDAG)))))[1:length(edgedat)]
edgedf[,2] <- factor(append(unlist(lapply(strsplit(names(edgedat), "\\|"), function(x){x[2]})), unlist(names(igraph::V(metaDAG)))))[1:length(edgedat)]
