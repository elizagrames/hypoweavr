...
# Note to self: need to convert everything to lapply for length-indeterminate
...
input.data <- read.csv("./data/Study_characteristics_new.csv")[1:296,]
...
wrong_hab <- c("farm fields", "not forest", "matrix surrounding", "shrubland")
notNA <- c("North America")
nofull <- c("needPDF")
wrong_pop <- c("focal species", "specialist")
wrong_time <- c("overwinter", "season", "transient", "post-fledg")

wrong_comp <- c("comparing", "time for space", "thinning")
existing_data <-
  c(
    "already included",
    "data comes",
...
prop.table(sort(table(reasons_excl)))
...
#
# png("./figures/study_locations_new2.png", width=12,height=8,units="in",res=1200)
# par(xpd=T)
# sp::plot(forested, col=paste(palette[4], "22", sep=""), axes=F, box=F, legend=F)
# sp::plot(canada, add=T, border="grey85")
# sp::plot(usa, add=T, border="grey85")
#
# points(studysites, col="white", pch=20, lwd=0, cex=1.75)
# points(studysites, col=paste(palette[1], "bb", sep=""), pch=20, cex=1.75, lwd=0)
...
sum(rowSums(matrix_hits[,1:3])>0)
119/145 # anthropogenic edges

# Forest type ------------------------------------------------------------------

forest_types <- input.data$forest_class
forest_types <- gsub("/", " ", forest_types)
forest_types <- gsub("-", " ", forest_types)

sort(table(forest_types))
sort(table(forest_types))/(145-38)

input.data$forest_class <- forest_types
...
bird_lookup <- cbind(bird_lookup, birdlife$`Family name`[match(bird_lookup[,3], birdlife$`Scientific name`, )])

birdlife$`Family name`[match(bird_lookup[,3], birdlife$`Scientific name`)]
...
dat2$prop <- round(dat2$count/(145-47), 3)
...
#unique(trimws(unlist(processes[123:134])))[!(unique(trimws(unlist(processes[123:134]))) %in% synonyms$processes)]
...
study_DAGS <- create_DAGS(input.data, 22)
...
#
#
# # Number of new factors is less than 1 per every two studies (<0.50)
# # Number of new pathways is less than n.factors at the corresponding time step (k-1)^2
...
# lines(apply(poismodfact, 2, mean), col=palette[2], lwd=4)
#
# ci <- apply(poismodpath, 2, quantile, c(0.025, 0.975), na.rm=T)
# newx <- seq(1, length(study_DAGS), 1)
# polygon(x=c(newx, rev(newx)), y=c(ci[1,], rev(ci[2,])),
#         col=paste(palette[4], "44", sep=""), border = F)
# lines(apply(poismodpath, 2, mean), col=palette[4], lwd=4)
#
# legend("topright",
...
# polygon(x=c(newx, rev(newx)), y=c(ci[1,], rev(ci[2,])),
#         col=paste(palette[2], "66", sep=""), border = F)
# lines(apply(modfact, 2, mean), col=palette[2], lwd=4)
#
# ci <- apply(modpath, 2, quantile, c(0.025, 0.975), na.rm=T)
# newx <- seq(1, length(study_DAGS), 1)
# polygon(x=c(newx, rev(newx)), y=c(ci[1,], rev(ci[2,])),
#         col=paste(palette[4], "44", sep=""), border = F)
# lines(apply(modpath, 2, mean), col=palette[4], lwd=4)
#
# legend("topright",
#        legend=c("Mean", "95% confidence interval", "Pathways", "Factors"),
#        lwd=c(2, NA, NA, NA, NA), pch=c(NA, 15, 15, 15),
...
edgedat <- lookup_widths(study_DAGS, metaDAG)
edgedf <- array(dim=c(length(edgedat), 3))


edgedf[,1] <- unlist(lapply(strsplit(names(edgedat), "\\|"), function(x){x[1]}))
edgedf[,2] <- unlist(lapply(strsplit(names(edgedat), "\\|"), function(x){x[2]}))
edgedf[,3] <- as.numeric(edgedat)
edgedf <- as.data.frame(edgedf)
edgedf[,3] <- as.numeric(edgedf[,3])
head(edgedf)

nodedat <- igraph::V(metaDAG)
nodedf <- array(dim=c(length(nodedat), 3))
nodedf[,1] <- levels(factor(names(nodedat)))
nodedf[,2] <- names(nodedat)
nodedf[,3] <- rep(1, nrow(nodedf))
nodedf <- as.data.frame(nodedf)
nodedf[,3] <- as.numeric(nodedf[,3])
nodedf[,4] <- sqrt(as.numeric(igraph::strength(metaDAG)))
colnames(nodedf) <- c("id", "label", "group", "value")
colnames(edgedf) <- c("from", "to", "degree")

nodedf$font.size <- ceiling(nodedf[,4])*4

E(metaDAG) %in% E(study_DAGS[[129]])
E(metaDAG)[1:3]


selection_mat <- array(dim=c(nrow(edgedf), length(study_DAGS)))

for(i in 1:length(study_DAGS)){
  hits <- attr(igraph::E(metaDAG), "vnames") %in% attr(igraph::E(study_DAGS[[i]]), "vnames")
  selection_mat[,i] <- hits
}

# for the app, we need:
# 1) metadata for each study

labeldat <- read.csv("./edge-area-sensitivity/data/Study_characteristics_new.csv")[1:145,]

tail(labeldat$Reference)
tail(input.data$Title)

input.data$label <- labeldat$Reference
#
# save(input.data, file="./edge-area-sensitivity/data/metadata.rda")
# # including taxa and matrix matrices for multiple
# save(bird_lookup, bird_dat2, file="./edge-area-sensitivity/data/bird_info.rda")
# save(matrix_hits, file="./edge-area-sensitivity/data/matrix_info.rda")

# 2) selection matrix for edges
# save(selection_mat, file="./edge-area-sensitivity/data/edge_lookups.rda")
#
#
# # 3) node and edge dataframes
# save(nodedf, edgedf, file="./edge-area-sensitivity/data/graph_attr.rda")

# then we can subset by study characteristics
# and recalculate the network metrics easily
# we can also plot the other types of graphs
# no need for recreating the DAGs

dim(selection_mat)


#selected <- rowSums(selection_mat[,which(input.data$forest_class == "oak hickory" & input.data$type=="article")])
selected <- rowSums(selection_mat)

edges <- edgedf[selected>0,]
nodes <- nodedf[nodedf$id%in%edges$from | nodedf$id %in% edges$to,]
nodes$label <- levels(factor(nodes$label))
...
nodes$value <- igraph::strength(igraph::graph.edgelist(cbind(edges$from, edges$to)))
edges$group <- rep(2, nrow(edges))
edges$group[edges$value>=17] <- 1
edges$color <- "#cacaca"
edges$color[edges$group==1] <- palette[4]

strong_hyps <- as.character(unlist(edges[edges$value>=17, 1:2]))
edges <- edges[order(edges$value),]

nodes$font.size[nodes$label%in%strong_hyps]<- 80
nodes$group[nodes$label%in%strong_hyps]<- 1
nodes$group[!nodes$label%in%strong_hyps]<- 2
nodes$color <- "#cacaca"
nodes$color[nodes$group==1] <- palette[2]
nodes$font.size[nodes$font.size!=80] <- 1
# network 2
library(visNetwork)
...
visNetwork(nodes, edges, width = "100%") %>%
  ...
  visNodes(
    scaling = list(
      min = min(nodes$value)+20,
      max = max(nodes$value)+20,
...
strengths <- ((sort(igraph::page_rank(metaDAG)$vector)))
strengthlabels <- names(strengths)
strengthlabels[strengths<(0.011275392)] <- ""
oddstrengths <- evenstrengths <- strengthlabels
oddstrengths[gtools::even(1:length(strengthlabels))] <- ""
evenstrengths[gtools::odd(1:length(strengthlabels))] <- ""
oddstrengths[108:115] <- strengthlabels[108:115]
evenstrengths[108:115] <- ""

plot((strengths), pch=20, cex=0.75, xlim=c(0,170), axes=F,  log="y", ylim=c(0.002, 0.1),
     ylab="Relative PageRank", xlab="Factors and Processes")
axis(2)
text(strengths, labels=oddstrengths, pos=2, offset=.25, cex=0.6)
text(strengths, labels=evenstrengths, pos=4, offset=.25, cex=0.6)

strengths <- ((sort(igraph::strength(metaDAG))))
strengthlabels <- names(strengths)
strengthlabels[strengths<(14)] <- ""
oddstrengths <- evenstrengths <- strengthlabels
oddstrengths[gtools::even(1:length(strengthlabels))] <- ""
evenstrengths[gtools::odd(1:length(strengthlabels))] <- ""
oddstrengths[113:115] <- strengthlabels[113:115]
evenstrengths[113:115] <- ""

plot((strengths), pch=20, cex=0.75, xlim=c(0,170), axes=F, log="y", ylim=c(1,100),
     ylab="Relative Strength", xlab="Factors and Processes")
axis(2)
text(strengths, labels=oddstrengths, pos=2, offset=.25, cex=0.6)
text(strengths, labels=evenstrengths, pos=4, offset=.25, cex=0.6)
...
visNetwork(nodes, edges, width = "100%") %>%
  visIgraphLayout(layout = "circle") %>%
  visNodes(
    shape = "circle",  label=nodes$label,
    scaling = list(
      min = min(nodes$value),
      max = max(nodes$value), label=list(enabled="true", min=1, max=30),
      customScalingFunction = 'function (min,max,total,value) {
                                                   if (max === min) {
                                                       return 0.5;
                                                   }
                                                   else {
                                                    console.log("Rule Min:" + min);
                                                    console.log("Rule Max:" + max);
                                                       var scale = 1 / (max - min);
                                                       return Math.max(0,(value*15 - min)*scale);
                                                   }
                                               }'
    ),
    color = list(
  ...
  visEdges(
    # shadow = FALSE, value=edges$degree,  scaling = list(
    #   min = min(edges$degree),
    #   max = max(edges$degree), label=list(enabled="true", min=1, max=10),
    #   customScalingFunction = 'function (min,max,total,value) {
    #                                                if (max === min) {
    #                                                    return 0.5;
    #                                                }
...
getwd()
dir.create("./figures/newchanges3")
setwd("./figures/newchanges3/")
svg()
par(las=1, pty="s")
plot_features(calc_features(cumulative_DAGs, 5),
              input.data$firstyear, xlab="Year", ymax=350)

plot_features(calc_features(yearwindow_DAG, 5),
              input.data$firstyear, xlab="Year", ymax=120)
...
plot_features(calc_features(latband_DAG, 10),
              input.data$latband, xlab="Latitude", ymax=200)
plot_features(calc_features(longband_DAG, 19),
              input.data$longband, xlab="Longitude", ymax=100)
