...
# only works on output from clean_path
...
generate_graph <- function(paths){
  ...
  study.graph <- igraph::graph_from_data_frame(blank, directed=TRUE)
  count.paths <- c()
  ...
  study <- unlist(lapply(strsplit(as.character(x), ";"), trimws))
    ...
    for (j in 1:length(study)){
      ...
      path.DAG <- igraph::graph_from_data_frame(path.edges, vertices=path.nodes, directed=TRUE)
...
remove_redundancy <- function(graph){
...
graph_metrics <- function(x, metric="page_rank", return.df=FALSE){
  ...
  subfunc <- function(z){
    if(class(z)=="igraph"){
      ...
      if(metric%in%c("edge.betweenness")){
...
# 10: Generate Shiny app

# Provide example code to do this, but not a function
# Too many moving parts for the function to produce a useful app
# Or does it basically just need the matrix and characteristics?
# Then define the selection functions based on data type? i.e. slider or check?
